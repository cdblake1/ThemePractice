...
public class FrameTest {
	...
	public static void main(String[] args) throws RiotApiException {
		Integer[] storedItemIds = { 1300, // berserker greaves enchantment:
											// furor
		...
		for (int k = 3; k < 4; k++) {
			...
			frameList.addAll((Collection<? extends Frame>) matchOne
					.getTimeline().getFrames());
			...
			for (Event event : frameList.get(counter).getEvents()) {
				...
				if (event.getParticipantId() == sumPartId
						&& event.getEventType().equals("ITEM_UNDO")) {
					if (event.getItemId() == 2010) {
			...
			for (int i = 3; i < frameList.size(); i++) {
				for (int j = 0; j < frameList.get(i).getEvents().size(); j++) {
					...
					if (curEvent.getParticipantId() == sumPartId
							&& curEvent.getEventType().equals("ITEM_PURCHASED")) {
			...
			JsonObject jObject = JsonFileBuilder.jsonObjectBuilder(
					"vs EnemyChamp", startingItems, coreItems, endGame);
			try {
				FileWriter file = new FileWriter(
						"C:\\Riot Games\\League of Legends\\Config\\Champions\\Cassiopeia\\Recommended\\"
								+ "ItemSet_" + matchRef.get(k) + ".json");
